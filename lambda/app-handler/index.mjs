...
async function sendCircuitBreakerAlert(details) {
  if (!SNS_TOPIC_ARN) {
    console.warn('SNS_TOPIC_ARN not configured, skipping notification');
    return;
  }

  const { diffMinutes, threshold, apiId, stageName, deletionSuccess, deletionError } = details;

  const subject = '🚨 iSpeed Circuit Breaker Activated';

  const message = `Circuit Breaker Alert
===================

The iSpeed circuit breaker has been triggered due to excessive write frequency.

Details:
- Timestamp: ${new Date().toISOString()}
- Time Since Last Write: ${diffMinutes.toFixed(2)} minutes
- Threshold: ${threshold} minutes
- API ID: ${apiId}
- Stage Name: ${stageName}
- API Stage Deletion: ${deletionSuccess ? 'Success' : 'Failed'}
${deletionError ? `- Deletion Error: ${deletionError}` : ''}

Action Required:
The API has been disabled to prevent further writes. You will need to manually redeploy the API stage to restore service.

Redeploy command:
npm run cdk deploy

---
Generated by iSpeed Circuit Breaker
`;
  ...
  try {
    ...
    console.log('Circuit breaker notification sent successfully');
  } catch (error) {
...
async function handleAppend(event) {
  ...
  if (diffMinutes < THRESHOLD_MINUTES) {
    ...
    // Send notification BEFORE attempting deletion
    await sendCircuitBreakerAlert({
      diffMinutes,
      threshold: THRESHOLD_MINUTES,
      apiId: API_ID,
      stageName: STAGE_NAME,
      deletionSuccess: false, // Will send another if needed
      deletionError: null,
    });
